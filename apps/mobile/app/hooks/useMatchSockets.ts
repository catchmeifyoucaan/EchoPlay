...
import { DebateRound, MatchRoomState, ReactionTally, useMatchStore } from '../state/matchStore';
...
export const useMatchSockets = () => {
  ...
  useEffect(() => {
    ...
    const onRoundStarted = (payload: { round: DebateRound; timer?: { duration: number; remaining: number; endsAt?: string } }) => {
      handleRoundStarted(payload);
    };
    const onRoundEnded = (payload: { roundId: string; number: number }) => {
